...
public:
	...
	uint32_t sizeType(std::string typeToCount) {
		for (uint32_t i; i < meshes.size(); i++) {
			uint32_t count = 0;
			if (auto mesh = std::get_if<MeshTexture>(&meshes[i])) {
				if (mesh->flag.compare(typeToCount) == 0) {
					count++;
				}
			}
			else if (auto mesh = std::get_if<Skybox>(&meshes[i])) {
				if (mesh->flag.compare(typeToCount) == 0) {
					count++;
				}
			}
...
private:
	...
	void cleanupSwapChain() {
		...
		vkDestroyDescriptorPool(device, descriptorPoolTexture, nullptr);
	}
	...
	void createImageViews() {
		...
		for (uint32_t i = 0; i < swapChainImages.size(); i++) {
			swapChainImageViews[i] = createImageView(swapChainImages[i], swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_VIEW_TYPE_2D, 1, 1);
		}
	}
	...
	void createRenderPass() {
		...
		//The previous color attachment can not be drawn in its current state to the screen because of the multisampling applied, so we need to resolve it in this way:
	...
	void createDescriptorSetLayoutUBO() {
	...
	void createDescriptorSetLayoutTexture() {
		...
		if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &descriptorSetLayoutTexture) != VK_SUCCESS) {
			throw std::runtime_error("failed to create descriptor set layout!");
		}
	}
	...
	void createDepthResources() {
		...
		createImage(swapChainExtent.width, swapChainExtent.height, 1, std::nullopt, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory, 1, msaaSamples);
		depthImageView = createImageView(depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, VK_IMAGE_VIEW_TYPE_2D, 1, 1);
	}

	void createColorResources() {
		VkFormat colorFormat = swapChainImageFormat;

		createImage(swapChainExtent.width, swapChainExtent.height, 1, std::nullopt, colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, colorImage, colorImageMemory, 1, msaaSamples);
		colorImageView = createImageView(colorImage, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_VIEW_TYPE_2D, 1, 1);
	}
	...
	void createTextureImageSTB(uint32_t meshIndex) {
		...
		mesh.mipIndex = (uint32_t)mipLevels.size() - 1;
		...
		createImage(texWidth, texHeight, 1, std::nullopt, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage[meshIndex], textureImageMemory[meshIndex], mipLevels[mesh.mipIndex], VK_SAMPLE_COUNT_1_BIT);

		transitionImageLayout(textureImage[meshIndex], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, mipLevels[mesh.mipIndex]);
		...
		generateMipmaps(textureImage[meshIndex], VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mipLevels[mesh.mipIndex]);
	}
	...
	void createTextureSkyboxSTB(uint32_t meshIndex) {
		...
		createImage(texWidth, texHeight, 6, VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage[meshIndex], textureImageMemory[meshIndex], 1, VK_SAMPLE_COUNT_1_BIT);
		transitionImageLayout(textureImage[meshIndex], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 6, 1);
		...
		transitionImageLayout(textureImage[meshIndex], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 6, 1);
	}
	...
	void createTextureImageView() {
		...
		for (int i = 0; i < scene.meshes.size(); i++) {
			if (auto mesh = std::get_if<MeshTexture>(&scene.meshes[i])) {
				textureImageView[i] = createImageView(textureImage[i], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_VIEW_TYPE_2D, 1, mipLevels[mesh->mipIndex]);
				mesh->imageViewIndex = i;
			}
			if (auto mesh = std::get_if<Skybox>(&scene.meshes[i])) {
				textureImageView[i] = createImageView(textureImage[i], VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_VIEW_TYPE_CUBE, 6, 1);
				mesh->imageViewIndex = i;
			}
		}
	}
	...
	VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, VkImageViewType type, uint32_t layerCount, uint32_t mipLevel) {
	...
	void createImage(uint32_t width, uint32_t height, uint32_t arrayLayers, std::optional<VkImageCreateFlags> flags, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory, uint32_t mipLevel, VkSampleCountFlagBits numSamples) {
		VkImageCreateInfo imageInfo{};
		imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
		imageInfo.imageType = VK_IMAGE_TYPE_2D;
		if (flags.has_value()) {
			imageInfo.flags = flags.value();
		}
	...
	void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t layerCount, uint32_t mipLevel) {
		VkCommandBuffer commandBuffer = beginSingleTimeCommands();
	...
	void createDescriptorSetsUBO() {
		std::vector<VkDescriptorSetLayout> layouts(swapChainImages.size(), descriptorSetLayoutUBO);
		...
		descriptorSetsUBO.resize(swapChainImages.size());
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			...
			duboInfo.buffer = dynamicUniformBuffers[i];
			duboInfo.offset = 0;
			duboInfo.range = VK_WHOLE_SIZE;

			std::array<VkWriteDescriptorSet, 2> descriptorWrites{};

			descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
			descriptorWrites[0].dstSet = descriptorSetsUBO[i];
			...
			descriptorWrites[1].dstSet = descriptorSetsUBO[i];
			descriptorWrites[1].dstBinding = 1;
			descriptorWrites[1].dstArrayElement = 0;
			descriptorWrites[1].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC;
			descriptorWrites[1].descriptorCount = 1;
			descriptorWrites[1].pBufferInfo = &duboInfo;
	...
	void updateDynamicUniformBuffer(uint32_t currentImage) {
		...
		void* data;
		vkMapMemory(device, dynamicUniformBuffersMemory[currentImage], 0, bufferSize, 0, &data);
		memcpy(data, dubo.model, bufferSize);
		vkUnmapMemory(device, dynamicUniformBuffersMemory[currentImage]);

		alignedFree(dubo.model);
	}
